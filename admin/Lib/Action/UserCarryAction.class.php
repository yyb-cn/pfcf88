...
class UserCarryAction extends CommonAction{
	...
	public function update(){
		...
		if (false !== $list) {
			...
			if($data['status']==1){
				...
				modify_account(array("money"=>-($vo['money']+$vo['fee']),"lock_money"=>-($vo['money']+$vo['fee']),"pfcfb"=>-($vo['pfcfb']),"","lock_pfcfb"=>-($vo['pfcfb'])),$vo['user_id'],"提现成功");
				...
				//驳回
				modify_account(array("lock_money"=>-($vo['money']+$vo['fee']),"lock_pfcfb"=>-($vo['pfcfb'])),$vo['user_id'],"提现失败");
	...
	public function export_csv()
	{
		...
		$arr[$k+1]=array($k+1,$v['bank_name'],$v['region_lv2_name'],$v['region_lv3_name'],$v['bankzone'],$v['real_name'],"'".$v['bankcard'],$v['money'],$v['phone'],"'".$v['desc'],to_date($v['create_time'],'Y-m-d'),to_date($v['update_time'],'Y-m-d'),$v['user_name']);
		}
