...
class spaceModule extends SiteBaseModule
{
	...
	public function level(){
		...
		if (!$GLOBALS['tmpl']->is_cached('page/space_credit.html', $cache_id))	
		{
	    	...
	    	//还清
	    	$level_point['repay_success'] = $GLOBALS['db']->getRow("SELECT sum(point) as total_point,count(*) AS total_count FROM ".DB_PREFIX."user_log WHERE log_info='还清借款' AND user_id=".$this->space_user['id']);
	    	//逾期
	    	$level_point['impose_repay'] = $GLOBALS['db']->getRow("SELECT sum(point) as total_point,count(*) AS total_count FROM ".DB_PREFIX."user_log WHERE log_info='逾期还款' AND user_id=".$this->space_user['id']);
	    	//严重逾期
	    	$level_point['yz_impose_repay'] = $GLOBALS['db']->getRow("SELECT sum(point) as total_point,count(*) AS total_count FROM ".DB_PREFIX."user_log WHERE log_info='严重逾期还款' AND user_id=".$this->space_user['id']);
