...
{
	$deal_info = $GLOBALS['db']->getRow("select *,(start_time + enddate*24*3600 - ".get_gmtime().") as remain_time,(load_money/borrow_amount*100) as progress_point from ".DB_PREFIX."deal where id = ".$id);
	...
	if($deal_info['deal_status']!=3){
		...
		if($deal_info['progress_point'] >=100 || $data['progress_point'] >=100){
			...
			elseif(($deal_info['deal_status']==4&&$deal_info['repay_start_time']>0) || ($deal_info['deal_status']==2 && $deal_info['repay_start_time']>0 && $deal_info['repay_start_time'] <= get_gmtime())){
				$repay_info =  $GLOBALS['db']->getRow("SELECT sum(repay_money) As all_repay_money ,MAX(repay_time) AS last_repay_time FROM ".DB_PREFIX."deal_repay WHERE deal_id=$id");
				if($repay_info){
				...
				if($data['repay_money'] >= round($has_repay_money,2)){
					$data['deal_status'] = 5;
				}
				else{
					$data['deal_status'] = 4;
	...
	if($deal_info['deal_status'] ==3 || $data['deal_status']==3){
		...
		foreach($r_load_list as $k=>$v){
			modify_account(array("money"=>$v['money']),$v['user_id'],"流标返还");
...
function sys_user_status($user_id,$is_cache = false,$make_cache=false){
	...
	if($data==false){
		...
		//总借款额
		$data['borrow_amount'] = $GLOBALS['db']->getOne("SELECT sum(borrow_amount) FROM ".DB_PREFIX."deal  use index(idx_0) WHERE deal_status in(4,5) AND user_id=$user_id AND publish_wait = 0");
		//已还本息
		$data['repay_amount'] = $GLOBALS['db']->getOne("SELECT sum(repay_money) FROM ".DB_PREFIX."deal_repay WHERE user_id=$user_id");
				
		//发布借款笔数
		$data['deal_count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal use index(idx_1) WHERE user_id=$user_id AND publish_wait = 0");
		//成功借款笔数
		$data['success_deal_count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal use index(idx_0) WHERE deal_status in (4,5) AND user_id=$user_id AND publish_wait = 0");
		//还清笔数
		$data['repay_deal_count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal use index(idx_0) WHERE deal_status = 5 AND user_id=$user_id AND publish_wait = 0");
		//未还清笔数
		...
		//逾期费用
		$data['yuqi_impose'] = $GLOBALS['db']->getOne("SELECT sum(repay_money) FROM ".DB_PREFIX."deal_repay use index(idx_0) WHERE user_id=$user_id AND status in(2,3)");
		
		//逾期次数
		$data['yuqi_count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_repay use index(idx_0) WHERE user_id=$user_id AND status = 2");
		//严重逾期次数
		$data['yz_yuqi_count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_repay use index(idx_0) WHERE user_id=$user_id AND status = 3");
		
		//待还本息
		$data['need_repay_amount'] = 0;
		//待还管理费
		$data['need_manage_amount'] = 0;
		$deals = $GLOBALS['db']->getOne("SELECT id FROM ".DB_PREFIX."deal  use index(idx_0) WHERE deal_status in(4,5) AND user_id=$user_id AND publish_wait = 0"); 
		if($deals){
